...
/**
 * UI tests for NdkBinderClient MainActivity
 * Tests the service binding and IPC communication flow via JNI/NDK
 */
...
public class MainActivityUITest {
    ...
    public void testServiceIsInstalled() {
        ...
        assertTrue("JavaBinderService must be installed for this test", serviceExists);
    }
    ...
    public void testServiceConnectionAndCommunication() {
        ...
        // Verify that a response was returned (non-empty string after "Returned : ")
        onView(withId(R.id.sample_text))
                .check(matches(withText(containsString("Returned :"))));
    }

    @Test
    public void testServiceResponseContainsComplexType() {
        // Wait for service communication to complete
        textIdlingResource.waitForTextChange();

        // Verify the response contains ComplexType data
        // The native code calls returnComplexType and should display the returned object
        onView(withId(R.id.sample_text))
    ...
    public void testComplexTypeValuesInResponse() {
        ...
        // These match the values passed in native-lib.cpp talkToService()
        onView(withId(R.id.sample_text))
